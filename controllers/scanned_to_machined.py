...
def read_scanned_pdf(pdf_path, location):
    file_name = pdf_path.replace(' ', '_')
    file_name_without_ext = os.path.basename(file_name).split('.')[0]
    doc_dir_location = location
    #doc_dir_location = os.path.join(location, 'uploads', file_name_without_ext)
    print("LOCATION-->", location)
    page_dir_location = os.path.join(doc_dir_location, 'pages')
    print("page_dir_location", page_dir_location)
    ...
    parse_file = os.path.join( text_dir, 'stitched.txt')
    ...
    return {'text_file_path': text_dir, 'stitched_pdf_path': stitched_file, 'parse_file': parse_file}
    print('Completed')
